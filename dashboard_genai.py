# ~/Desktop/kafka-fraud-detection/dashboard_genai.py
# ============================================================
# 🛡️ Real-Time Fraud Detection Dashboard — with GenAI Explanations
# ============================================================
# Run:  streamlit run dashboard_genai.py
# Requires: pip install streamlit altair joblib openai shap
# ============================================================
...
st.set_page_config(page_title="Real-Time Fraud Monitoring + GenAI", layout="wide", page_icon="🛡️")
...
st.markdown('<div class="subtitle">Kafka ➜ Spark ➜ ML Model ➜ GenAI Explanations</div>', unsafe_allow_html=True)
st.markdown("")
...
with st.sidebar:
    ...
    openai_key = st.text_input(
        "OpenAI API Key", value=env_key, type="password",
        help="Enter your OpenAI API key for GenAI fraud explanations", placeholder="sk-...",
    )
    if openai_key:
    ...
    st.divider()
    st.markdown("""
    **Run Order (in terminals):**
    1. ZooKeeper
    2. Kafka broker
    3. `python spark_streaming_consumer.py`
    4. `python kafka_paysim_producer.py`
    5. `streamlit run dashboard_genai.py`
    """)
...
3. **Pattern Match**: What known fraud pattern does this resemble (e.g., account draining, money laundering, balance manipulation)
...
# ---------------------------------------------------
# LOAD ALERTS — reads .csv AND Spark .part-* files
# ---------------------------------------------------

def load_alerts() -> pd.DataFrame:
    files = glob.glob(os.path.join(DATA_DIR, "*.csv"))
    files += glob.glob(os.path.join(DATA_DIR, ".part-*"))
    files += glob.glob(os.path.join(DATA_DIR, "part-*"))
    ...
    for f in files:
        ...
        try:
            ...
            if not df_piece.empty:
                dfs.append(df_piece)
        except (pd.errors.EmptyDataError, pd.errors.ParserError, UnicodeDecodeError, Exception):
            continue
...
st.caption("Select a flagged transaction to get an AI-powered explanation of why it was flagged.")
...
with genai_col1:
    ...
    selected_txn = st.selectbox("Choose a transaction to analyze:", options, index=0 if options else None)
    if selected_txn:
...
cols_to_show = [c for c in ["transaction_id", "ts", "type", "amount", "ml_score", "severity", "is_fraud_ml", "rule_flag", "is_fraud_final"] if c in df.columns]
...
if "severity" in df_latest.columns:
    styled = df_latest.style.map(color_severity, subset=["severity"])
...
if "ts" in df.columns and "ml_score" in df.columns:
    ...
    if not df_time.empty:
        line_chart = alt.Chart(df_time).mark_line(point=True).encode(
            x=alt.X("ts:T", title="Time"), y=alt.Y("ml_score:Q", title="ML score"),
...
`is_fraud_ml` = ML flagged, `rule_flag` = business rule triggered, `is_fraud_final` = fraud alert (ML OR rules)
